...
/*
 * pg_prewarm(regclass, text, text, integer, integer)
 *
 * The first argument is the relation to be prewarmed; the second is the name
 * of the relation fork to be prewarmed.  The third argument controls how
 * prewarming is done; legal options are 'prefetch', 'read', and 'buffer'.  The
 * fourth and fifth arguments specify the first and last block to be prewarmed.
 ...
 *
 * SELECT pg_prewarm('relation', 'main', 'prefetch', NULL, NULL);
 */
Datum
pg_prewarm(PG_FUNCTION_ARGS)
{
	...
	else
	{
		...
		if (first_block < 0)
	        ereport(ERROR,
					(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
					 errmsg("starting block number may not be negative")));
	}
	if (PG_ARGISNULL(4))
